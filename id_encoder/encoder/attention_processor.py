...
class IDAttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(
        ...
        id_scale=1.0,
        unet_stage=None,
        hidden_states_height=None,
        hidden_states_width=None,
    ):
        ...
        def spectral_decompose(features: torch.Tensor, hidden_states_height = hidden_states_height, hidden_states_width = hidden_states_width, low_freq_radius: float = 0.1):
            """
            Args:
                features: Tensor, shape [B, H*W, C]，且 H*W 是完全平方数（H=W）。
                low_freq_radius: float, 相对归一化频率的低频半径 (0 < low_freq_radius < 0.5)。

            Returns:
                low_feat, high_feat: Tensor, shape [B, H*W, C]，分别对应低频和高频部分。
            """
            B, HW, C = features.shape
            # side = int(HW**0.5)
            # assert side * side == HW, "输入的 HW 不是完全平方数"
        ...
        # ##################################
        # if id_embedding is not None and unet_stage == "down":
        #     id_embedding, _ = spectral_decompose(id_embedding, low_freq_radius=0.3)
        # ##################################
        residual = hidden_states
        ...
        if id_embedding is not None:
            ...
            if not ORTHO and not ORTHO_v2:
                ...
                # import pdb; pdb.set_trace() # debug IDAttnProcessor2_0-frequency
                if unet_stage == "down":
                    id_hidden_states, _ = spectral_decompose(id_hidden_states, low_freq_radius=0.3)
                    # pass
                ##################
                hidden_states = hidden_states + id_scale * id_hidden_states
            elif ORTHO_v2:
