...
class UNet2DConditionModel(
    ...
    def forward(
        ...
        # import pdb; pdb.set_trace() # debug gligen参数形状
        if cross_attention_kwargs is not None and cross_attention_kwargs.get("gligen", None) is not None:
            cross_attention_kwargs = cross_attention_kwargs.copy()
            gligen_args = cross_attention_kwargs.pop("gligen")
            objs = self.position_net(
                interaction_cfg = gligen_args["interaction_cfg"] if gligen_args.get("interaction_cfg") is not None else None,
                id_cfg = gligen_args["id_cfg"] if gligen_args.get("id_cfg") is not None else None,
                masks = gligen_args["masks"],
            )
            '''
            Args:
                id_embedding.shape torch.Size([2, 30, 10, 2048])
                encoder_hidden_states.shape torch.Size([2, 77, 2048])
                gligen_args["masks"].shape torch.Size([2, 30])
            Returns:
                fused_states.shape torch.Size([2, 77, 2048])
            '''

            cross_attention_kwargs["gligen"] = {"objs": objs,
                                                "t_emb": t_emb,
                                                "task_type": gligen_args["task_type"],
                                                }
            # import pdb; pdb.set_trace() # debug 参数形状

        # 3. down
        cross_attention_kwargs["gligen"]["unet_stage"] = "down"
        cross_attention_kwargs["unet_stage"] = "down"
        ...
        # 4. mid
        cross_attention_kwargs["gligen"]["unet_stage"] = "mid"
        cross_attention_kwargs["unet_stage"] = "mid"
        ...
        # 5. up
        cross_attention_kwargs["gligen"]["unet_stage"] = "up"
        cross_attention_kwargs["unet_stage"] = "up"
        for i, upsample_block in enumerate(self.up_blocks):
